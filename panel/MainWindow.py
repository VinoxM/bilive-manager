...
from components.Changeable import ChangeableTextEdit
...
class UiMainWindow(QtWidgets.QWidget):
    ...
    def __init__(self):
        ...
        self.ws_status = 0
        self.minimized = 1
        self.room_info = {'uname': '', 'uid': '', 'room_id': '', 'area_id': '', 'area_name': '', 'title': ''}
    ...
    def setupUi(self, main_window):
        ...
        self.scrollArea.setProperties(auto_remove=False)
    ...
    def call_real_room_id(self, room_info):
        if room_info.get('err', '') == '':
            ...
            if room_id != 0:
                ...
                self.room_info['area_id'] = room_info.get('area_id')
                self.room_info['area_name'] = room_info.get('area_name')
                self.room_info['title'] = room_info.get('title')
                self.print_log('已获取 {} 的真实直播间: {}'.format(self.room.text(), room_id))
    ...
    # ws信息回调 -- 直播
    def call_bili_ws_live_status_change(self, status):
        self.print_log('直播已{}:\n{} ({})'.format('开始' if status == 1 else '结束',
                                              self.room_info['uname'],
                                              self.room_info['room_id']))
    ...
    def call_get_last_barrage(self, obj):
        if obj.get('err', None):
            ...
            for elem in message:
                self._signal_bili_ws_receive.emit({
                    'cmd': 'MESSAGE',
                    'data': elem
                })
    ...
    def call_bili_ws_live_info_change(self, info):
        title = info['title']
        area_id = info['area_id']
        area_name = info['area_name']
        if title != self.room_info['title']:
            self.room_info['title'] = title
            self.print_log('主播刚更新了直播标题:\n{}'.format(title))
        if area_id != self.room_info['area_id']:
            self.room_info['area_id'] = area_id
            self.room_info['area_name'] = area_name
            self.print_log('主播刚更新了直播区域:\n{}'.format(area_name))
